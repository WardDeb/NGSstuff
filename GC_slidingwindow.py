...
for seqrecord in SeqIO.parse(fasta, "fasta"):
    ...
    for i in range(1,(int(nowindow)+1)):
        ...
        oneid = str(seqrecord.id.split(sep="_")[0] + seqrecord.id.split(sep="_")[1] + '_' + seqrecord.id.split(sep="_")[8] ) +' ' + str((i*500)-500) + ' ' + str((i*500)) + ' ' + str(test)
        print(oneid)
    testje = SeqUtils.GC(seqrecord.seq[((int(nowindow)*500)):])
    twoid = str(seqrecord.id.split(sep="_")[0] + seqrecord.id.split(sep="_")[1] + '_' + seqrecord.id.split(sep="_")[8] ) + ' ' + str((nowindow*500)) + ' ' + str(length) + ' ' + str(round(testje,1))
    print(twoid)
